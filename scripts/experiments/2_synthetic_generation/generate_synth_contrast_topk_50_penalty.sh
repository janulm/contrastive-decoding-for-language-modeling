...
TOP_K=50
DATASET_NAME="synth_contrast_early_500_topk_${TOP_K}_penalty"
CD_STRATEGY="contrast_with_top_k"
TITLE="contrast-early-500-topk-${TOP_K}-penalty"
ALPHA=0.1
...
TRAIN_SCRIPT="/cluster/home/janulm/thesis/ms-thesis/scripts/experiments/3_8000series_all/contrast_early_500_topk_${TOP_K}_penalty.sh"
...
cat > "${GEN_SCRIPT}" << 'EOF'
#!/bin/bash
#SBATCH --job-name=__DATASET_NAME__
#SBATCH --ntasks=1
...
python generate_synthetic_data_better.py \
  ...
  --penalty_value "__PENALTY_VALUE__"
EOF
...
sed -i \
  ...
  -e "s|__PENALTY_VALUE__|${PENALTY_VALUE}|g" \
  "${GEN_SCRIPT}"
...
cat > "${COMB_SCRIPT}" << 'EOF'
#!/bin/bash
#SBATCH --job-name=sdg_combine__DATASET_NAME__
#SBATCH --ntasks=1
...
sed -i \
  -e "s|__LOG_DIR__|${LOG_DIR}|g" \
  -e "s|__OUTPUT_DIR__|${OUTPUT_DIR}|g" \
  -e "s|__TRAIN_SCRIPT__|${TRAIN_SCRIPT}|g" \
  -e "s|__DATASET_NAME__|${DATASET_NAME}|g" \
  "${COMB_SCRIPT}"
